 ...
 */
export function checkedAddRowBefore(
  editorState: EditorState,
  dispatch?: EditorView["dispatch"]
): boolean {
 ...
 */
export function maybeDeleteTable(
  editorState: EditorState,
  dispatch?: EditorView["dispatch"]
) {
  const selection = editorState.selection;
  const selectedNodeSecondAncestor: Node | undefined = selection.$from.node(
