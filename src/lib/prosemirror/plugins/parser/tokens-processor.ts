...
export const basicPunctuationData: Record<string, BasicPunctuationData> = {
  ...
  heading: {
    ...
    count: (token) => {
      ...
      return [
        0,
        token.raw.trim().length - (token as Tokens.Heading).text.length,
      ]; // Deals with the heading with dashes or equals format
    },
    parseInside: true,
  },
};
...
export function processTokenForRanges(
  ...
  if (htmlRawLength > 0 && token.type !== "html" && excludedTypes.length <= 1) {
    htmlStack[htmlRawLength - 1].afterContent =
      htmlStack[htmlRawLength - 1].afterContent.concat(token);
  }
