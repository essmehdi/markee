...
const TOKEN_HANDLER_MAP: TokenHandlerMap = {
  ...
  blockquote: (token) => {
    return mdSchema.nodes.blockquote.create(
      null,
      deserializeTokens(token.tokens as MarkedToken[])
    );
  },
  html: (token) => {
        ...
        token.header.map((headerCell, index) => {
          const cellAttrs = {
            align: token.align[index]
              ? TABLE_CELL_ALIGNMENT_MAP[token.align[index]]
              : "start",
          };
          return mdSchema.nodes.table_header.create(
            cellAttrs,
            mdSchema.text(headerCell.text)
          );
        })
      ),
  ...
  list: (token) => {
    ...
    const listType =
      listAttrs === null
        ? mdSchema.nodes.bullet_list
        : mdSchema.nodes.ordered_list;
    return listType.create(
      listAttrs,
      token.items.map((listItem) => {
        const listItemAttrs =
          listAttrs === null ? { checked: listItem.checked ?? null } : null;
        return mdSchema.nodes.list_item.create(
          listItemAttrs,
          deserializeTokens(listItem.tokens as MarkedToken[])
        );
      })
    );
  },
...
function getTextNodesWithBreaks(
  token: Tokens.Heading | Tokens.Paragraph | Tokens.Text
): Node[] {
  const content =
    token.type === "heading" ? token.raw.replace(/\s+$/, "") : token.text;
