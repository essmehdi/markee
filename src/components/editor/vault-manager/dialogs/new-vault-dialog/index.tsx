    ...
    <>
      ...
      <div className="w-full space-y-1.5">
        ...
        <Select
          value={newVaultType}
          onValueChange={
            setNewVaultType as React.Dispatch<
        ...
        >
          ...
          <SelectContent>
            {Object.keys(vaultTypes).map((vaultType) => {
              const vaultTypeOption =
                vaultTypes[vaultType as keyof typeof vaultTypes];
              return (
                <SelectItem
                  key={vaultType}
                  value={vaultType}
                  disabled={vaultTypeOption.disabled}
                >
        ...
        <p className="text-sm text-neutral-400">
          Local vault is only available with browsers that support the File
          System Access API
        </p>
      </div>
      {newVaultType === "local" && <LocalVaultForm closeDialog={closeDialog} />}
      {newVaultType === "browser" && (
        <BrowserVaultForm closeDialog={closeDialog} />
      )}
    </>
  );
}
