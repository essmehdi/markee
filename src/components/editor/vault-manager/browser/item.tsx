...
export default function BrowserItem({
	...
	const isCurrentSource =
		source &&
		vault?.id === source.vault.id &&
		source.file.absolutePath === node.data.absolutePath;
	...
	const onItemClick = (event: MouseEvent) => {
		...
		if (
			...
			!node.tree.props.disableMultiSelection
		) {
						...
						<div className="shrink-0">
							{node.data.type === "file" ? (
								<File
									...
									weight={isCurrentSource ? "fill" : undefined}
								/>
							) : node.isOpen ? (
