...
export default function LocalVaultForm({ closeDialog }: LocalVaultFormProps) {
	...
	const { mutate, isPending } = useMutation({
		mutationFn: ({ name, rootHandle }: { name: string; rootHandle: FileSystemDirectoryHandle }) =>
			LocalVault.saveNewVault(name, rootHandle),
		onSuccess: () => {
	...
	const handleSubmit = useCallback(
		(event: FormEvent) => {
			...
			mutate({ name: vaultDetails.name, rootHandle: vaultDetails.rootHandle });
		},
		[vaultDetails]
	);
		...
		<form className="space-y-4" onSubmit={handleSubmit}>
			<div className="w-full space-y-1.5">
				...
				<Input
					...
					onChange={(e) => setVaultDetails((old) => ({ ...old, name: e.target.value }))}
					required
				/>
			...
			<div className="w-full space-y-1.5">
				...
				<Button id="vault-type" className="flex w-full" type="button" variant="outline" onClick={openFolderPicker}>
					<Folder />
					{vaultDetails.rootHandle === null ? "Choose a folder" : vaultDetails.rootHandle.name}
				</Button>
