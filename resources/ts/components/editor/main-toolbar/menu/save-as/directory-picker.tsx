...
export default function DirectoryPicker({ vault, onSelect }: DirectoryPickerProps) {
		...
		<div style={style} className={className}>
			...
			>
				<div className="shrink-0">
					{node.data.type === "file" ? (
						<File className="text-neutral-500" weight={node.isSelected ? "fill" : undefined} />
					) : node.isOpen ? (
				...
				<p data-selected={node.isSelected} className="line-clamp-1 data-[selected=true]:font-bold">
					{node.data.name}
				</p>
			</div>
		</div>
