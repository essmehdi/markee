...
export default function SaveAsDialog({ open, onOpenChange }: SaveAsDialogProps) {
	const [vault, setVault] = useState<Vault | null>(null);
	const [selectedDirectory, setSelectedDirectory] = useState<VaultDirectory | null>(null);
				...
				<div className="space-y-5">
					...
					{vault && (
						<>
							...
							<Input name="file-name" value={fileName} onChange={(e) => setFileName(e.target.value)} />
						</>
					)}
					<div className="space-y-2">
						<p data-visible={error !== ""} className="text-destructive-foreground data-[visible=true]:invisible">
							{error}
						</p>
						<Button className="h-full" disabled={selectedDirectory === null} onClick={saveFile}>
							Save
						</Button>
					</div>
				</div>
